...
Module ArasUtil
    ...
    Function Addfile(file As String, name As String) As Aras.IOM.Item
        Dim it As Aras.IOM.Item = Nothing
        If file <> "" Then
            Dim fileitem As Aras.IOM.Item = inn.newItem("File", "add")
            fileitem.setProperty("filename", name)
    ...
    Public Function AllParts() As Aras.IOM.Item
        Dim Parts As Aras.IOM.Item = inn.newItem()
        Dim aml As String = "<Item type='PART' action='get' select='id,item_number,state'></Item>"
        Parts.loadAML(aml)
        Parts = Parts.apply()
        Return Parts
    End Function

    Public Sub otherfile(ByVal part As Aras.IOM.Item, ByVal file As Aras.IOM.Item)
        ...
        Dim partfile As Aras.IOM.Item = inn.newItem
        Dim amlstring1 As String = "<Item type='Part File2'  action='add' ><related_id>" + file.getID() + "</related_id><source_id>" + part.getID() + "</source_id></Item>"
        partfile.loadAML(amlstring1)
